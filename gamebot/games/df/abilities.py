...
class DFAbilities:
    ...
    # 模板文件路径（跟 abilities.py 同目录，方便不动代码就能改 prompt）
    PROMPT_TEMPLATE_PATH = Path(__file__).parent / "prompt.md"

    @classmethod
    def _load_prompt_template(cls) -> str:
        """读取 prompt.md 模板。每次调用都重读，方便热修改。"""
        if not cls.PROMPT_TEMPLATE_PATH.exists():
            return "你是三角洲行动助手。{tool_list}（缺少 prompt.md 模板）"
        text = cls.PROMPT_TEMPLATE_PATH.read_text(encoding="utf-8")
        # 剥掉开头的 HTML 注释块（如果有），让最终 prompt 干净
        import re as _re
